    ...
    <>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 lg:py-16">
        <div className="grid gap-6 lg:grid-cols-3">
          {dietPlans.map((plan) => (
            <div key={plan.slug} className="flex flex-col rounded-3xl border border-border bg-card p-6 shadow-sm">
              ...
              <div className="mt-5 space-y-4">
                {plan.meals.map((meal) => (
                  <div key={meal.time} className="border-l-2 border-primary/30 pl-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-primary">{meal.time}</p>
      ...
      <section className="bg-secondary/40 py-12 lg:py-16">
        <div className="mx-auto max-w-4xl px-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-2xl font-semibold text-foreground sm:text-3xl">Weight Loss Tips</h2>
          </div>
          <ul className="mt-6 grid gap-3 sm:grid-cols-2">
            {weightLossTips.map((tip, i) => (
              <li key={tip} className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
      ...
      <section className="mx-auto max-w-6xl px-4 py-14 text-center">
        ...
        <p className="mx-auto mt-3 max-w-xl text-muted-foreground">
          Combine these diet plans with SehatSlim herbal capsules to control cravings and support fat burning.
        </p>
        <div className="mt-6">
