...
const highlights = [
  { icon: Leaf, title: '100% Herbal', text: 'Plant-based ingredients with no harsh chemicals.' },
  { icon: Flame, title: 'Burns Fat', text: 'Supports metabolism and natural fat burning.' },
  { icon: HeartPulse, title: 'Curbs Cravings', text: 'Helps you eat less without feeling weak.' },
  { icon: Wallet, title: 'Cash on Delivery', text: 'Pay only when the parcel reaches you.' },
]
      ...
      <div className="grid items-center gap-10 lg:grid-cols-2">
        <div className="order-2 lg:order-1">
          <div className="grid grid-cols-2 gap-4">
            {highlights.map((h) => (
              <div key={h.title} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="order-1 lg:order-2">
          <p className="text-sm font-semibold uppercase tracking-wide text-primary">Our Product</p>
          <h2 className="mt-2 text-balance font-serif text-3xl font-semibold text-foreground sm:text-4xl">
            {product.name}
          </h2>
          <p className="mt-4 text-pretty leading-relaxed text-muted-foreground">{product.tagline}</p>
